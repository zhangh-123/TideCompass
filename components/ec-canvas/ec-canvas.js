...
function getSystemInfoSafe() {
  try {
    if (typeof wx.getWindowInfo === 'function' && typeof wx.getAppBaseInfo === 'function') {
      var w = wx.getWindowInfo() || {};
      var b = wx.getAppBaseInfo() || {};
      return {
  ...
  try {
    ...
    return { SDKVersion: '', pixelRatio: 1 };
  }
}
...
Component({
  ...
  methods: {
    init: function init(callback) {
      var version = getSystemInfoSafe().SDKVersion;
    ...
    initByNewWay: function initByNewWay(callback) {
      ...
      query.select('.ec-canvas').fields({ node: true, size: true }).exec(function (res) {
        ...
        var canvasDpr = getSystemInfoSafe().pixelRatio;
