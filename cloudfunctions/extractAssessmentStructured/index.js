...
const STRUCTURED_SYSTEM_PROMPT = `你是财务信息结构化抽取助手，仅根据对话中用户（user）明确陈述的事实提炼数据，禁止编造数字。
忽略助手（assistant）的推测性表述，除非用户已确认。
...
4) 月薪、家庭月总收入写入 monthly_income_yuan；固定月支出合计写入 monthly_expense_yuan；不清楚填 null。
